...
SAVE_DIR = r"F:\00_Coal_Gangue_Recognition_Paper\3.30_DINOv2\LoRA_DINOv2\deepmlp_eval_results\field"

                              
MODEL_NAME = "facebook/dinov2-base"

                               
ADAPTER_DIR = r"F:\00_Coal_Gangue_Recognition_Paper\3.30_DINOv2\LoRA_DINOv2\权重file\lab_enhance\runs_deepmlp_layernorm_gelu\best_lora_adapter"

                    
HEAD_WEIGHT_PATH = r"F:\00_Coal_Gangue_Recognition_Paper\3.30_DINOv2\LoRA_DINOv2\权重file\lab_enhance\runs_deepmlp_layernorm_gelu\best_mlp_head.pth"
...
class LoRADINOv2DeepMLPLayerNormGELU(nn.Module):
...
def build_model():
    model = LoRADINOv2DeepMLPLayerNormGELU(
        model_name=MODEL_NAME,
        adapter_dir=ADAPTER_DIR,
        num_classes=NUM_CLASSES,
    )

    checkpoint = torch.load(HEAD_WEIGHT_PATH, map_location="cpu")
...
def main():
    ...
    if not os.path.exists(ADAPTER_DIR):
        print("Error: LoRA adapter directory not found")
        print(ADAPTER_DIR)
        return

    if not os.path.exists(HEAD_WEIGHT_PATH):
        print("Error: classification head weight file not found")
        print(HEAD_WEIGHT_PATH)
        return
    ...
    print("Model type:LoRA-DINOv2-DeepMLP-LN-GELU")
    ...
    csv_path = os.path.join(SAVE_DIR, "predictions_deepmlp_layernorm_gelu.csv")
