...
def build_loaders(data_root, processor, batch_size=8, num_workers=0):
    train_set = DINOv2ImageFolder(os.path.join(data_root, "train"), processor)
    val_set = DINOv2ImageFolder(os.path.join(data_root, "val"), processor)
    test_set = DINOv2ImageFolder(os.path.join(data_root, "test"), processor)
...
def save_best_checkpoint(model, processor, save_dir, epoch, best_val_f1):
    ...
    torch.save(
        ...
        os.path.join(save_dir, "best_linear_head.pth"),
    )


def load_best_checkpoint(model, save_dir, device):
    ckpt = torch.load(os.path.join(save_dir, "best_linear_head.pth"), map_location=device)
...
def main():
    ...
    save_dir = r"F:\00_Coal_Gangue_Recognition_Paper\3.30_DINOv2\LoRA_DINOv2\权重file\lab_enhance"
